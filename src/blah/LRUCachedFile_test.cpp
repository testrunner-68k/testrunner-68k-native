...
TEST(LRUCachedFile, OpenFailsWithInvalidFileName) {
    ...
    ASSERT_EQ((LRUCachedFile*)0, LRUCachedFile_open("invalid_filename.txt"));
}

TEST(LRUCachedFile, OpenSucceedsWithValidFileName) {

    LRUCachedFile* lruCachedFile = LRUCachedFile_open(TestFileName);
    ASSERT_NE((LRUCachedFile*)0, lruCachedFile);
